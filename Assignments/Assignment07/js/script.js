...
let frequencies = [
  523.25,587.33,659.25,392.00,440.00,
];

let silence = [1,2,3,4];

let nDuration = [2000,500,700,320,250,2500,420,500,304];
// The synth
let synth;
// The sound files
let kick;
...
function mousePressed() {
  //This if statement stops the music from restarting again when the user clicks a second time.
  if (mouseCtrl === 1){
  //Play note
  let newNote = setTimeout(playNote,200);
...
function playNote() {
  ...
  //Roll randomizer to see if we should have a silence or not
  if (silence[Math.floor(Math.random() * silence.length)] === 2){
    synth.stop();
  }
  //Else, play note normally (1/4 chance of having a silence)
  else {
    synth.play();
  }
  //Endlessly play more notes with varied duration based on the array of possible durations
  let newNote = setTimeout(playNote,(nDuration[Math.floor(Math.random() * nDuration.length)]));
